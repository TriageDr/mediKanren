...
(define gene-list
  (command-line
   ...
   ["--optimize-2"        (; show help on separate lines
                          ...
                          (optimize-level 2)]
   #:args (gene2run) ; expect one command-line argument: <gene2run>
   ; return the argument as a gene to run
   (list gene2run)))
...
(define export-path/proviral
  "/home/ozborn/code/repo/mediKanrenAutomatedQueries/alzheimer/proalz_gene_2hop/")
  ...
  (time
   (lambda (curie gene-ls-name)
     (cond
       ...
       (else
        (let* ()
          ...
          (define 1-hop-affector-gene/path
            (format "~a1HOP-AFFECTOR-GENES--ALLp-->~a.tsv" export-path gene-ls-name))
          (define 1-hop-affector-drug/path
            (format "~a1HOP-AFFECTOR-DRUGS--ALLp-->~a.tsv" export-path gene-ls-name))
          ...
          (define 1-hop-affector-gene/path-csv
            (format "~a1HOP_AFFECTOR_GENES_ALLp_PROVIRAL_GENES.csv" export-path))
          (define 1-hop-affector-drug/path-csv
            (format "~a1HOP_AFFECTOR_DRUGS_ALLp_PROVIRAL_GENES.csv" export-path))
          ...
          (define 1-hop-affector-gene-specific/path
            (format "~a1HOP-AFFECTOR-GENES--ALLp-->~a.tsv" export-path curie-str/EXPORT))
          (define 1-hop-affector-drug-specific/path
            (format "~a1HOP-AFFECTOR-DRUGS--ALLp-->~a.tsv" export-path curie-str/EXPORT))
          ...
          (define 2-hop-affector-drug/path
            (format "~a2HOP-AFFECTOR-DRUGS--ALLp-->1HOP-AFFECTOR-GENES-->ALLp-->~a.tsv" export-path gene-ls-name))
          ...
          (define 2-hop-affector-drug-specific/path
            (format "~a2HOP-AFFECTOR-DRUGS--ALLp-->~a.tsv" export-path curie-str/EXPORT))
          ...
          (define 2-hop-affector-drug-->1hop-gene-->target-gene-paths/path
            (format "~a2HOP-AFFECTOR-DRUGS--ALLp-->1HOP-AFFECTOR-GENES--ALLp-->PROVIRAL-GENES.tsv" export-path))
          ...
          (format "~a2HOP-AFFECTOR-GENES--ALLp-->1HOP-AFFECTOR-GENES--ALLp-->~a.tsv" export-path curie-str/EXPORT))
          ...
          (define 2-hop-affector-gene-paths/path
            (format "~a2HOP-AFFECTOR-GENES--ALLp-->1HOP-AFFECTOR-GENES--ALLp-->PROVIRAL-GENES.tsv" export-path))
	  ...
	  #|INDIVIDUAL FILE EXPORTS PER CONCEPT QUERY|#

         (define 2-hop-affector-drug-specific/path-per-path
           (format "~a2HOP-AFFECTOR-DRUGS--ALLp-->~a.tsv" export-path curie-str/EXPORT))
         (define 2-hop-affector-drug-specific/port-per-path
           (open-output-file 2-hop-affector-drug-specific/path-per-path #:exists 'append))

         (export-column-headers
          column-headers/1hop
          2-hop-affector-drug-specific/port-per-path
          2-hop-affector-drug-specific/path-per-path)
         ...
         (define 1-hop-affector-gene/path-per-curie
           (format "~a1HOP-AFFECTOR-GENES--ALLp-->~a.tsv" export-path curie-str/EXPORT))
         (define 1-hop-affector-drug/path-per-curie
           (format "~a1HOP-AFFECTOR-DRUGS--ALLp-->~a.tsv" export-path curie-str/EXPORT))

         (define 1-hop-affector-gene/port-per-curie
           (open-output-file 1-hop-affector-gene/path-per-curie #:exists 'append))
         (define 1-hop-affector-drug/port-per-curie
           (open-output-file 1-hop-affector-drug/path-per-curie #:exists 'append))

         (export-column-headers
          column-headers/1hop
          1-hop-affector-gene/port-per-curie
          1-hop-affector-gene/path-per-curie)
         ...
         (export-column-headers
          column-headers/1hop
          1-hop-affector-drug/port-per-curie
          1-hop-affector-drug/path-per-curie)
...
(start-function gene-list "ALZHEIMER-GENE-LIST")
